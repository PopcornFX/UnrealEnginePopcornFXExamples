...
struct	SFragGeometry
{
	vec2	m_UV0;
	vec2	m_UV1;
// Particle has basic UVs on top of atlas UVs
	vec2	m_RawUV0;
...
#if defined(HAS_UVDistortions) || defined(HAS_AlphaMasks)
struct  SMaskDistortion
{
#	if defined(HAS_UVDistortions)
    vec2 m_UVDistortion1;
    vec2 m_UVDistortion2;
#	endif
...
#		if defined(HAS_Atlas)
	...
	fGeom.m_UV1 *= fragRect1.xy;
	fGeom.m_UV1 += fragRect1.zw;
#		else
...
#		else
	fGeom.m_UV1 = VEC2_ZERO;
#		endif
#		if defined(FINPUT_fragRawUV0) // Particle has basic UVs on top of atlas UVs
	fGeom.m_RawUV0 = fInput.fragRawUV0;
#		else
	fGeom.m_RawUV0 = fGeom.m_UV0;
#		endif
#	endif
